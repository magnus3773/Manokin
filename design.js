...
(function() {
    ...
    // Styles (Manokin: Grün + Dunkelgrau)
    ...
    widgetStyles.textContent = `
      .chat-assist-widget {
        ...
        --chat-color-light: var(--chat-widget-light, rgba(31,35,38,0.25));
        --chat-color-surface: var(--chat-widget-surface, #1F2326); /* Mitte = Grau */
        --chat-color-text: var(--chat-widget-text, #ffffff);
        --chat-color-text-light: var(--chat-widget-text-light, #cbd5e1);
        --chat-color-border: var(--chat-widget-border, rgba(255,255,255,0.08));
        --chat-shadow-sm: 0 1px 3px rgba(0,0,0, 0.18);
        --chat-shadow-md: 0 4px 6px rgba(0,0,0, 0.22);
        --chat-shadow-lg: 0 10px 15px rgba(0,0,0, 0.28);
        --chat-radius-sm: 8px;
        --chat-radius-md: 12px;
        --chat-radius-lg: 20px;
      ...
      .chat-assist-widget .chat-window {
        ...
        width: 380px;
        height: 580px;
        background: var(--chat-color-surface); /* Fenster-Hintergrund = Grau */
      ...
      .chat-assist-widget .chat-bubble.bot-bubble {
        ...
        border: 1px solid rgba(0,0,0,0.08); border-bottom-left-radius: 6px;
      }
      ...
      .chat-assist-widget .chat-textarea:focus {
        ...
        box-shadow: 0 0 0 3px rgba(75, 146, 7, 0.25);
      }
      .chat-assist-widget .chat-textarea::placeholder { color: #94a3b8; opacity: 1; }
      ...
      /* Footer bleibt grün, aber ohne Text/Link */
    ...
    const defaultSettings = {
      ...
      branding: {
        ...
        poweredBy: { text: '', link: '' } // leer gelassen
      },
      style: {
        primaryColor: '#4B9207',
        secondaryColor: '#3E7A05',
        position: 'right',
        backgroundColor: '#1F2326', // Mitte = Grau
        fontColor: '#ffffff'
      },
      suggestedQuestions: []
    };
    ...
    async function submitMessage(messageText) {
      ...
      try {
        ...
        errorMessage.textContent = "Tut mir leid, ich konnte Ihre Nachricht nicht senden.";
    ...
    // Input autosize & submit
    messageTextarea.addEventListener('focus', () => {
      setTimeout(() => { messagesContainer.scrollTop = messagesContainer.scrollHeight; }, 250);
    });
